...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO employees (full_name, national_id, residence, role, shift_start, shift_end, start_date, username, password,path_qr_Employee) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
$stmt->execute([$full_name, $national_id, $residence, $role, $shift_start, $shift_end, $start_date, $username, $hashed_password, $qr_code_path]);
...
<body>
    ...
    <div class="container mt-1 bg-dark text-light p-2 shadow-lg">
        <h4 class="alert alert-info text-center">إضافة موظف جديد</h4>
                        ...
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <select name="role" id="role" class="form-control " required>
                                    <option value="general_manager" disabled class="mt-4 text-center text-light bg-dark">مدير عام </option>
                                    <option value="supervisor" class="mt-4 text-center text-light bg-dark">مشرف</option>
                                    <option value="accountant" class="mt-4 text-center text-light bg-dark">محاسب</option>
                                    <option value="marketer" class="mt-4 text-center text-light bg-dark">مسوق</option>
                                    <option value="legal_accountant" class="mt-4 text-center text-light bg-dark">محاسب قانوني (محامي)</option>
                                    <option value="it_department" class="mt-4 text-center text-light bg-dark">قسم IT</option>
                                    <option value="programming_department" class="mt-4 text-center text-light bg-dark">قسم البرمجة</option>
                                </select>
                            </div>
                        </div>
